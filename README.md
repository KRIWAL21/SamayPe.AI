...
Powered by **Google Gemini 2.5 Flash** and backed by a **MongoDB Atlas Cloud Database**, the system breaks monolithic goals into achievable subtask roadmaps, scores temporal risk in real time, and—when a schedule slips—autonomously compresses durations and renegotiates deadlines into realistic focus blocks. This intelligence meets users where they already live: inside a professional **ClickUp & Todoist inspired web dashboard** or bi-directionally over **WhatsApp messaging** (including voice notes and photo attachments), ensuring zero cognitive friction.
...
1. **Google Gemini 2.5 Flash (`@google/genai` Official SDK)**
   * **Structured Function Calling (`/api/decompose`):** Converts raw user goals into JSON subtask arrays with precise minutes estimation and risk scoring.
...
* **Real-Time Agentic Thinking HUD:** A live telemetry modal showing Gemini's reasoning pipeline, step-by-step cognitive decomposition, and constraint-checking steps in real time.

### Tier 2: ClickUp & Todoist Inspired Professional Architecture (NEW ✨)
* **Standalone Execution Inbox (`/inbox`):** Dedicated zero-clutter capture workspace separated from the main dashboard. Features real-time task filtering, one-click subtask toggles, and integrated **Voice Goal Capture** wired directly to Gemini cognitive reasoning and MongoDB persistence.
...
* **Dynamic Personalized User Profiles (`/profile`):** Customizable display names, avatars, professional roles (`Software Engineer`, `Hackathon Builder`, `Research Scientist`), and execution modes. Powers dynamic time-based dashboard greetings (`Good morning/afternoon/evening, {name}`).

### Tier 3: Production Cloud Persistence & Multichannel Capture
* **MongoDB Atlas Cloud Engine (`/api/tasks`):** Enterprise-grade cloud persistence ensuring user profiles, tasks, subtasks, and AI recommendations are reliably stored and synchronized across all sessions.
...
### Tier 4: Context-Aware Visibility & Coaching
...
### Tier 5: Habit Tracking, Community & Evaluation HUD
...
1. Open the **Live Cloud Run URL** or run locally (`npm run dev`) and explore the **Dashboard**. Note the personalized time-of-day greeting (`Good morning/afternoon/evening, {userName}`).
