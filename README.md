...
Powered by **Google Gemini 2.5 Flash** and backed by an enterprise-grade **MongoDB Atlas Cloud Database**, the system breaks monolithic goals into achievable subtask roadmaps, scores temporal risk in real time, and—when a schedule slips—autonomously compresses durations and renegotiates deadlines into realistic focus blocks. This intelligence meets users where they already live: inside an intuitive, high-performance web dashboard or bi-directionally over **WhatsApp messaging** (including voice notes and photo attachments), ensuring zero cognitive friction.
...
3. **Google Cloud Build & Container Registry**
   * Automated multi-stage container builds and container registry orchestration for seamless cloud deployment.
...
* **Goal Decomposition:** AI structured function calling turns high-level goals into ordered checklists with exact time estimates per subtask.
...
* **Agentic Thinking Visualizer HUD (`AgenticThinkingModal`):** A live telemetry modal showing the reasoning pipeline's step-by-step cognitive decomposition and constraint-checking steps in real time.
* **Standalone Execution Inbox (`/inbox`):** Dedicated zero-clutter capture view separated from the dashboard, featuring real-time task search filtering, one-click subtask toggles, and integrated Voice Goal capture.
...
* **Browser Voice Capture (`VoiceGoalButton.tsx` & `/inbox`):** Native Web Audio transcription feeds straight into the task-generation pipeline on desktop and inside the Execution Inbox.
...
* **Chronological Calendar Grid (`/calendar`):** Multi-domain visual timeline mapping active subtasks across a horizontal 7-day strip and vertical date groups (`Today`, `Tomorrow`, future dates), equipped with inline quick-add creators (`+ Add task / event`) under every specific date.
